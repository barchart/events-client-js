...
module.exports = (() => {
		...
		createEvents(events) {
			return Promise.resolve()
				.then(() => {
					...
					return Gateway.invoke(this._createEventEndpoint, { events: events.map((event) => EventSchema.TYPE.schema.format(event)) });
				});
		}
	...
	const createEventRequestInterceptor = (request) => {
		return Promise.all(request.data.events.map((event) => FailureReason.validateSchema(EventSchema.TYPE, event)))
			.then(() => {
