...
module.exports = (() => {
		...
		createEvents(events) {
			return Promise.resolve()
				.then(() => {
					...
					return Gateway.invoke(this._createEventEndpoint, events.map((event) => EventSchema.TYPE.format(event)));
				});
		}
	...
	const createEventRequestInterceptor = (request) => {
		return FailureReason.validateSchema(EventSchema.TYPE, request.data)
			.then(() => {
